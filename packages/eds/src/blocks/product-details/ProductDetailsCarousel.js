...
function OptimizedSources({ src, sizes, width, height, loading = 'lazy' }) {
	...
	return html`<${Fragment}>
		${sizes.map((size) => {
			...
			return html`${webpTag} ${jpgTag}`;
		})}
		<img
			height=${height}
			width=${width}
			src=${createUrlForWidth(src, width, false)}
			loading=${loading}
		/>
	<//>`;
}
...
export default class Carousel extends Component {
	...
	getImages() {
		...
		if (productImages) {
			this.thumbnailImages = productImages.map(
				(image) => `${image}?quality=100&bg-color=255,255,255`,
			);
			this.images = productImages.map(
				(image) => `${image}?width=700&quality=100&bg-color=255,255,255`,
			);
		}
	}
	...
	updateSlide(getNextIndex) {
		...
		const correctedNextIndex = Carousel.negativeModulo(
			nextIndex,
			this.images.length,
		);
		this.setState({
	...
	renderLabel() {
		...
		return html`<div class="amasty-wrapper">
			<div
				class=${amasty.position}
				style=${amasty.style}
				dangerouslySetInnerHTML=${{ __html: amasty.txt }}
			/>
		</div>`;
	}
	...
	render() {
		...
		if (!this.images || !this.thumbnailImages) {
			this.images = [
				`${window.origin}/${this.props.productImageFolder}/${getSkuFromUrl().toLowerCase()}.jpg`,
			];
				...
				<div class="carousel-thumbnails-wrapper">
					<div class="thumbnail-controls">
						<button
							...
							onClick=${() => this.updateThumbnailSlide((index) => index - 1)}
						>
					...
					<ul
						...
						style="transform: translateY(-${(this.state.thumbnailSlide === 0
							? 0
							: 1) *
							-252.7 +
						this.state.thumbnailSlide * 322.6}px)"
					>
						${!this.props.loading &&
						this.thumbnailImages.map(
							(image, i) =>
								html` <li
									key=${image}
									onClick=${() =>
										this.setState({ slide: i, thumbnailSlide: i })}
								>
									<picture>
										<${OptimizedSources}
											src=${image}
				...
				<div class="carousel-stage-wrapper">
					...
					html`
						<div class="main-controls">
							<button
								name="stage-prev"
								onClick=${() => this.updateSlide((index) => index - 1)}
							>
					...
					>
						...
						this.images.map(
							(image, i) => html`
								<li
									key=${image}
									active=${i === this.state.slide ? 'true' : 'false'}
								>
									<picture>
										<${OptimizedSources}
											src=${image}
