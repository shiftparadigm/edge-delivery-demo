	...
	(() => {
		...
		const Pr = ({ attribute: e }) => {
			...
			Cr = ({ filterData: e }) => {
				...
				const f = (e) => {
					...
					return `${t.currencySymbol ? t.currencySymbol : '$'}${e && parseFloat(r) * parseInt(e.toFixed(0), 10) ? (parseFloat(r) * parseInt(e.toFixed(0), 10)).toFixed(2) : 0}`;
				};
				return V('div', {
		...
		const Lr = ({
				...
				const s = ct(),
					...
					p = (e, t) => {
						if ('RangeBucket' === t.__typename) {
							...
							return `${r}${t?.from && parseFloat(e) * parseInt(t.from.toFixed(0), 10) ? (parseFloat(e) * parseInt(t.from.toFixed(0), 10)).toFixed(2) : 0}${t?.to && parseFloat(e) * parseInt(t.to.toFixed(0), 10) ? ` - ${r}${(parseFloat(e) * parseInt(t.to.toFixed(0), 10)).toFixed(2)}` : s.InputButtonGroup.priceRange}`;
						}
						if ('CategoryView' === t.__typename)
			...
			Tr = (e, t, r) => {
				const n = e.range,
					i = t || '1',
					a = r || '$';
				return `${a}${n?.from && parseFloat(i) * parseInt(n.from.toFixed(0), 10) ? (parseFloat(i) * parseInt(n.from?.toFixed(0), 10))?.toFixed(2) : 0}${n?.to && parseFloat(i) * parseInt(n.to.toFixed(0), 10) ? ` - ${a}${(parseFloat(i) * parseInt(n.to.toFixed(0), 10)).toFixed(2)}` : ' and above'}`;
			},
			Br = (e, t, r, n) => {
				if (n && r) {
										...
										children: t.map((e, t) =>
											V(
												'li',
												{
													...
													onMouseOver: () => d(t),
													className: `py-xs hover:bg-neutral-200 hover:text-neutral-900 ${t === l ? 'bg-neutral-200 text-neutral-900' : ''}}`,
													children: V('a', {
														className:
								...
								children: V('div', {
									...
									children: V('div', {
										...
										children: e.map((e, n) =>
											V(
												cn,
												{ image: e, carouselIndex: r, index: n, alt: t },
												n,
											),
										),
									}),
					...
					(M = ((e, t) => {
						...
						for (const i of e) {
							const e = on(i, { ...r, width: t }),
								a = [1, 2, 3].map(
									(e) =>
										`${on(i, { ...r, auto: 'webp', quality: 80, width: t * e })} ${e}x`,
								);
				...
				return c
					? V(w, {
							children: [
								...
								V('div', {
									className: `flex flex-row justify-between max-w-full ${e ? 'mx-auto' : 'mr-auto'} w-full h-full`,
									children: [
										V('div', {
											children: ((e, t, r) =>
		...
		const kn = () => {
				...
				return V('div', {
					...
					children: [
						V('button', {
							className: `flex items-center ${'gridview' === e ? 'bg-gray-100' : ''} ring-black ring-opacity-5  p-sm text-sm h-[32px] border border-gray-300`,
							onClick: () => r('gridview'),
							children: V(yn, { className: 'h-[20px] w-[20px]' }),
						}),
						V('button', {
							className: `flex items-center ${'listview' === e ? 'bg-gray-100' : ''} ring-black ring-opacity-5 p-sm text-sm h-[32px] border border-gray-300`,
							onClick: () => r('listview'),
										...
										children: t.map((e, t) =>
											V(
												'li',
												{
													'aria-selected': e.value === a?.value,
													onMouseOver: () => u(t),
													className: `py-xs hover:bg-neutral-200 hover:text-neutral-900 ${t === c ? 'bg-neutral-200 text-neutral-900' : ''}}`,
													children: V('a', {
														className:
							...
							? V('div', {
									...
									children: V('div', {
										...
										children: [
											...
											V('div', {
												className: `ds-widgets_results flex flex-col items-center ${t.categoryName ? 'pt-16' : 'pt-28'} flex-[75]`,
												children: [
													V(Nn, {
														facets: t.facets,
														totalCount: t.totalCount,
														...
														children:
															...
															V('div', {
																...
																children: V(mt, {
																	displayFilter: () => o(!0),
																	type: 'desktop',
																	title: `${n.Filter.showTitle}${e.filterCount > 0 ? ` (${e.filterCount})` : ''}`,
																}),
															}),
													}),
													t.loading
